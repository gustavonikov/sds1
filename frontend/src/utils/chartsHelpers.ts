...
export const buildBarSeries = (games: Game[], records: RecordItem[]) => {
    const mappedGames = games.map((game) => {
        const filteredGames = records.filter((item) => item.gameTitle === game.title && item.gamePlatform === game.platform);
...
export const getPlatformChartData = (records: RecordItem[]) => {
    ...
    const series = platforms.map((platform) => {
        const filteredGames = records.filter((item) => platform === item.gamePlatform);
